...
class Trainer:
    def __init__(self, options, joint_training=False):
        ...
        if self.use_pose_net:
            ...
            # self.models["pose"] = networks.PoseResNet(self.num_pose_frames)
            self.models["pose"] = networks.PoseViT(self.num_pose_frames)
            self.models["pose"].to(self.device)
            self.parameters_to_train += list(self.models["pose"].parameters())

        self.opt.predictive_mask = False
    ...
    def process_batch(self, inputs):
        ...
        losses = {"loss": torch.zeros(1, dtype=torch.float64).to(self.device)}
        for i in self.opt.frame_ids[1:]:
            losses["loss"] += self.compute_pose_loss(
                outputs[("axisangle", 0, i)][:, 0, 0],
                inputs[("gt_axisangle", 0, i)],
                outputs[("translation", 0, i)][:, 0, 0],
                inputs[("gt_translation", 0, i)],
            )

        losses["loss"] = losses["loss"].float()
        return outputs, losses

    def compute_pose_loss(self, pred_axis, target_axis, pred_trans, target_trans):
        return F.mse_loss(
            pred_axis, target_axis, size_average=True, reduce=True
        ) + 100 * F.mse_loss(pred_trans, target_trans, size_average=True, reduce=True)
            ...
            for f_i in self.opt.frame_ids[1:]:
                if f_i != "s":
                    ...
                    if self.opt.pose_model_type == "separate_resnet":
                        ...
                        pose_inputs = torch.stack(pose_inputs, 1)
